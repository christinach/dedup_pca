...
class IPCAOnEmbeddings:
    ...
    def _embedding_values(self):
        ...
        if self._embedding_values_cache is None:
            ...
            create_sample_embeddings_matrix.create_sample(batch_files=batch_files)
            embedding_df = pd.read_csv("embeddings_matrix/sample_10000_matrix.csv")
            self._embedding_values_cache = embedding_df.values
    ...
    def ipca_on_embeddings(self, batch_files):
        scaler = StandardScaler()
        for batch_file in batch_files:
            ...
            self.calculate_ipca.partial_fit(batch_data)
        ...
        for batch_file in batch_files:
            ...
            batch_data = pd.read_csv(batch_file).values
            batch_data = scaler.fit_transform(batch_data)
            x_ipca_batch = self.calculate_ipca.transform(batch_data)
        ...
        print(self.calculate_ipca.explained_variance_ratio_)

        print("Cumulative explained variance (IPCA):")
        print(np.cumsum(self.calculate_ipca.explained_variance_ratio_))
